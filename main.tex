...
\medskip\noindent\emph{Game 9:} Note that up to now we have not changed $P$'s algorithm before sending $(s,T)$ (in particular, Game 5 dealt with $H_2$ queries by $\adv$ and $P'$, but not $P$). In this game we modify it as follows: Sample $s \gets \{0,1\}^{3n}, T \gets \group$. If $H_2(\pw,T)$ has already been defined, abort. {\color{brown}Otherwise query $t := H_2(\pw,T)$, generate $(pk,sk) \gets \Gen$, and compute
\[
r := s \oplus t, \quad R := T/pk.
\]
