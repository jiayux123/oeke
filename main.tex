...
\[
\Dist_\env^{6,7} \leq \Adv_{\red_\text{ANO-1PCA}}^\text{ANO-1PCA}.
\]
...
Note that the aborting condition, which was originally introduced in Game 5, is removed (for the specific $H_2(\pw',T)$ query by $P'$). By an analysis similar to that in Game 5, the aborting probability in Game 7 is upper-bounded by $q_1/2^{3n}$.
