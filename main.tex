...
\begin{enumerate}
  ...
  \item On $(c^*,\tau^*)$ from $\adv$,
      ...
      \end{enumerate}
  \item On $\adv$'s $i$-th $\Hkey$ or $\Htag$ query, if it is in the form of $\Hkey(\pw,pk,c,(K')^*)$ for some $(K')^*$, output $(K')^*$; otherwise abort.
\end{enumerate}
...
The difference between Game 3 and Game 4 is that in Game 3 both $P$ and $P'$ use the same $pk$, whereas in Game 4 $P$ uses $pk$ and $P'$ uses an independent $pk'$. We construct a reduction $\red_\text{ANO-1PCA1}$ to the ANO-1PCA property of KEM. This reduction, in fact, is very similar to the reduction $\red_\text{OW-1PCA1}$ to the OW-1PCA property in Game 3, and we highlight their differences in {\color{blue}blue}:

\bigskip\noindent\textbf{\underline{Reduction $\red_\mathrm{ANO\text{-}1PCA1}$}:}

\medskip\noindent
On input $(pk,c)$,
\begin{enumerate}
  \item Invoke $\env$. On $(\NewSession, sid, P, P', \pw, \text{``initiator''})$ from $\env$, run $P$'s algorithm to compute $(s,T)$, {\color{blue}except that $\red_\text{ANO-1PCA1}$ uses its input $pk$ rather than $(pk,\star) \gets \Gen$}. (Note that computing $(s,T)$ does not require knowing $sk$.) Send $(s,T)$ to $\adv$.
  ...
  \item On $(c^*,\tau^*)$ from $\adv$,
      \begin{enumerate}
        ...
        \item \fbox{Query $\PC(sk,c^*,K^*)$}. \quad // {\color{blue}This is a valid query since we do this step only if $c^* \neq c$} \\
            ...
            If the result is 0, output $SK := \bot$ to $\env$.
      \end{enumerate}
  \item {\color{blue}When $\env$ outputs bit $b^*$, $\red_\text{ANO-1PCA1}$ also outputs $b^*$.}
\end{enumerate}

$\red_\text{ANO-1PCA1}$ uses its input $pk$ on the $P$ side and $c$ on the $P'$ side. If $b = 0$ then $c \gets \Encap(pk)$, i.e., both $P$ and $P'$ use the same $pk$, so $\red_\text{ANO-1PCA1}$ simulates Game 3; if $b = 1$ then $c \gets \Encap(pk')$, i.e., $P'$ uses an independently generated $pk'$, so $\red_\text{ANO-1PCA1}$ simulates Game 4. We have
\[
\Dist_\env^{3,4} \leq \Adv_{\red_\text{ANO-1PCA1}}^\text{ANO-1PCA}.
\]
