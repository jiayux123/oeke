...
Furthermore, in the case where $\pw \neq \pw' \land (s^*,T^*) = (s,T)$, when $P'$ queries $H_2(\pw',T)$, use the method described above to generate $pk',r',R'$ and program $H_1$. (Note that the case where $\pw = \pw' \land (s^*,T^*) = (s,T)$ has already been addressed in Game 3 and Game 4, and $P'$ does not need to make an $H_2$ query there.)
...
\medskip\noindent\emph{Game 9:} Note that up to now we have not changed $P$'s algorithm before sending $(s,T)$ (in particular, Game 5 dealt with $H_2$ queries by $\adv$ and $P'$, but not $P$). In this game we modify it as follows: Sample $s \gets \{0,1\}^{3n}, T \gets \group$. If $H_2(\star,T)$ has already been defined, abort. {\color{brown}Otherwise query $t := H_2(\pw,T)$, generate $(pk,sk) \gets \Gen$, and compute
\[
r := s \oplus t, \quad R := T/pk.
\]
...
\begin{itemize}
  \item If $H_2(\pw,T)$ has now been defined (via a query by either $\adv$ or $P'$), an $sk$ must have been defined together with $H_2(\pw,T)$ (see Game 5).
...
\begin{enumerate}
  ...
  \item \emph{$P$'s session key:} On $(c^*,\tau^*)$ from $\adv$ to $P$, we have made two changes:
      \begin{enumerate}
        ...
        \item Otherwise if $sk$ has not been defined yet, Game 10 queries $H_2(\pw,T)$, which generates $sk$ (see below). The remaining part is unchanged from the real world.
      \end{enumerate}
  \item \emph{$H_2$ queries:} When either $\adv$ or an honest party makes a fresh query $H_2(\pw^*,T)$ (let the result be $t^*$), Game 10 runs the following procedure: Generate $(pk^*,sk^*) \gets \Gen$, and compute
      \[
      r^* := s \oplus t^*, \quad R^* := T/pk^*.
      \]
      If $H_1(\pw^*,r^*)$ has already been defined and it is not $R^*$, abort. Otherwise program $H_1(\pw^*,r^*) := R^*$.
