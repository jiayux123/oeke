...
We have
\begin{gather*}
pk = \frac{T}{R} = \frac{T}{H_1(\pw,r)} = \frac{T}{H_1(\pw,s \oplus H_2(\pw,T))}, \\
pk' = \frac{T^*}{R^*} = \frac{T^*}{H_1(\pw,r^*)} = \frac{T^*}{H_1(\pw,s^* \oplus H_2(\pw,T^*))}.
\end{gather*}
There are two cases:
\begin{itemize}
...
\begin{enumerate}
  ...
  \item $\adv$ also queried $H_2(\pw^*,T)$, and
      \[
      s \oplus s^* = H_2(\pw^*,T) \oplus t^*.\footnote{Note that in this case $T^* \neq T$ since otherwise $T^* = T \Rightarrow H_2(\pw^*,T) = t^* \Rightarrow s^* = s \oplus H_2(\pw^*,T) \oplus t^* = s$, so $(s^*,T^*) = (s,T)$ which violates the condition in this game.}
      \]
\end{enumerate}
...
\paragraph{Inorrect password guess for $P'$.}
\mbox{}
