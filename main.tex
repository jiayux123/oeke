...
\begin{itemize}
  \item If $H_2(\pw,T)$ has now been defined (via a query by $\adv$), there must be a corresponding record $\left<\pw,\star,sk\right>$ (see Game 5).
...
\t \t SK := \bot \\
\t \text{fetch }\left<\pw,\star,sk\right>\text{ record} \< \< \< \< \\
\t \text{if none} \< \< \< \< \\
...
\text{if }P\text{ has not made this query} \\
\t (pk^*,sk^*) \gets \Gen \< \< \< \< \\
\t r^* := s \oplus t^* \< \< \< \< \\
\t R^* := T/pk^* \< \< \< \< \\
\t H_1(\pw^*,r^*) := R^* \< \< \< \< \\
...
\medskip\noindent\emph{Game 11:} In the case where $\neg(\pw = \pw' \land (s^*,T^*) = (s,T) \land c^* = c)$, if there is no $\left<\pw,sk\right>$ record, set $SK := \bot$ (i.e., replace the {\color{brown}brown} text in \Cref{fig:intermediate-game} with ``$SK := \bot$'').
...
\medskip\noindent\emph{Game 12:} Again in the case where $\neg(\pw = \pw' \land (s^*,T^*) = (s,T) \land c^* = c)$, find $\adv$'s $\Htag(\pw^*,pk,c^*,K^*)$ query whose result is $\tau^*$. (The difference is that Game 11 only goes over all of $\adv$'s queries in the form of $\Htag(\pw,pk,c^*,\star)$, whereas this game goes over all of $\adv$'s queries in the form of $\Htag(\star,pk,c^*,\star)$.) If there is none or more than one, set $SK := \bot$. Otherwise there is a unique such $\Htag(\pw^*,pk,c^*,K^*)$ query, and
\begin{itemize}
  \item If $\pw^* \neq \pw$, set $SK := \bot$.
...
If $\adv$ makes no $\Htag(\star,pk,c^*,\star)$ query resulting in $\tau^*$, then in particular $\adv$ makes no $\Htag(\pw,pk,c^*,\star)$ query resulting in $\tau^*$, so both Game 11 and Game 12 set $SK := \bot$. The probability that $\adv$ makes two different $\Htag(\star,pk,c^*,\star)$ queries both resulting in $\tau^*$ is upper-bounded by $\qtag(\qtag-1)/2^{2n+1}$. Now suppose $\adv$ makes a unique $\Htag(\pw^*,pk,c^*,K^*)$ query resulting in $\tau^*$:
\begin{itemize}
  \item If $\pw^* \neq \pw$, then $\adv$ does not make a $\Htag(\pw,pk,c^*,\star)$ query resulting in $\tau^*$ (because there is only one $\Htag(\star,pk,c^*,\star)$ query resulting in $\tau^*$, and it is on $\pw^*$), so both Game 11 and Game 12 set $SK := \bot$.
  \item If $\pw^* = \pw$, then there is no difference between Game 11 (which uses $\pw$) and Game 12 (which uses $\pw^*$).
\end{itemize}

We conclude that
\[
\Dist_\env^{11,12} \leq \frac{\qtag(\qtag-1)}{2^{2n+1}}.
\]

\emph{{\color{red}The game above is missing in \cite{EPRINT:ArrBarJar25}. The $\Htag$ collision case was ruled out in game $\mathsf{G}_2$ in \cite{EPRINT:ArrBarJar25} (as aborting condition [A1])\footnote{In our game hops, we ruled out $\Htag$ collisions for different $K^*$ values in Game 2, and then additionally rule out $\Htag$ collisions for different $(\pw^*,K^*)$ pairs in Game 12. We could have done both in Game 2 and avoided the additional loss here; however, we believe the current approach is clearer.}; however, conceptually speaking there still needs to be such a bridge game, to make sure that the game hops match the ideal world at the end. \\
...
\medskip\noindent\emph{Game 13:} In the case where $(s^*,T^*) \neq (s,T)$, find $\adv$'s $H_2(\pw^*,T^*)$ query\footnote{The notations are slightly messed up, since the $\pw^*$ here has nothing to do with what's called $\pw^*$ in Game 12. Essentially, the $\pw^*$ here is $\adv$'s guess of $P'$'s password (which is called $(\pw')^*$ in the simulator), whereas the $\pw^*$ in Game 12 is $\adv$'s guess of $P$'s password. The good news is that the $\pw^*$ in Game 12 is never reused once Game 12 is done, so there is no potential confusion.} (let the result be $t^*$) that satisfies \emph{either} of the following:
\begin{enumerate}
...
Summing up, we have
\[
\Dist_\env^{12,13} \leq \frac{q_2[q_1(q_1+q_2-2)+(q_2-1)(q_2-2)]}{2^{3n}}.
\]
...
\medskip\noindent\emph{Game 14:} Again in the case where $(s^*,T^*) \neq (s,T)$, if $\pw^*$ is defined in type (2) in Game 13 and $\pw^* = \pw'$, fetch record $\left<\pw^*,pk^*,\star\right>$ (there must be one since such a record is created when $\adv$ queries $H_2(\pw^*,T)$, as per Game 5) and set $pk' := pk^* \cdot (T^*/T)$.
...
pk^* = \frac{T}{R^*} = \frac{T}{H_1(\pw^*,r^*)} = \frac{T}{H_1(\pw^*,s \oplus t^*)} = \frac{T}{H_1(\pw^*,s \oplus H_2(\pw^*,T))}.
\]
The condition in this game specifies that $\pw^* = \pw'$, and type (2) in Game 13 specifies that $s \oplus H_2(\pw^*,T) = s \oplus H_2(\pw^*,T^*)$. This means that the denominators in the expressions of $pk'$ and $pk^*$ are equal, and thus $pk' = pk^* \cdot (T^*/T)$ --- exactly as in Game 14. We have
\[
\Dist_\env^{13,14} = 0.
\]
...
Let $\Query_3$ be the event that $\adv$ queries \emph{either} $\Hkey(K')$ \emph{or} $\Htag(\pw',\allowbreak pk',c,K')$. Clearly Game 14 and Game 15 are identical unless $\Query_3$ happens.

\section{Security Analysis of Other Versions}

\subsection{What Goes Wrong If You Don't Hash Everything?}
