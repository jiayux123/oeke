...
Let $\Query_1$ be the event that $\adv$ queries \emph{either} $\Hkey(\pw,pk,c,K')$ \emph{or} $\Htag(\pw,pk,c,K')$. Clearly Game 2 and Game 3 are identical unless $\Query_1$ happens. We upper-bound $\Pr[\Query_1]$ via a reduction $\red_\text{OW-1PCA1}$ to the OW-1PCA property of KEM:

\bigskip\noindent\textbf{\underline{Reduction $\red_\mathrm{OW-1PCA1}$}:} \quad \textcolor[gray]{0.5}{// \fbox{boxed text} indicates the single $\PC$ oracle query; same for reductions below}
