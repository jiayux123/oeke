...
\begin{enumerate}
  \item \emph{$P$-to-$P'$ message:} Game 10 outright samples $(s,T)$ at random.
  \item \emph{$P'$'s session key and $P'$-to-$P$ message:} On $(s^*,T^*)$ from $\adv$ to $P'$, if $(s^*,T^*) = (s,T)$, Game 10 outright samples $SK',\tau'$ at random. Regarding $c$, Game 10 generates $(pk',\star) \gets \Gen$ and computes $c \gets \Encap(pk')$. (If $(s^*,T^*) \neq (s,T)$ then there is no change from the real world.)
  \item \emph{$P$'s session key:} On $(c^*,\tau^*)$ from $\adv$ to $P$, we have made two changes:
      \begin{enumerate}
        \item If $\pw = \pw' \land (s^*,T^*) = (s,T) \land c^* = c$, Game 10 outright sets $SK := SK'$ if $\tau^* = \tau'$ and $SK := \bot$ otherwise.
        \item Otherwise if $\adv$ makes no $\Htag(\pw,pk,c^*,K^*)$ resulting in $\tau^*$, or more than one such query, Game 10 outright sets $SK := \bot$.
