...
\emph{{\color{red}This game corresponds to games $\mathsf{G}_6$ and $\mathsf{G}_7$ in \cite{EPRINT:ArrBarJar25}.}}

\medskip\noindent\emph{Game 3:} In the case where $\pw = \pw' \land (s^*,T^*) = (s,T)$, sample  $SK',\tau' \gets \{0,1\}^n$. ($c$ is still computed as in Game 2.)
...
\begin{enumerate}
  ...
  \item On $(c^*,\tau^*)$ from $\adv$,
      \begin{enumerate}
        ...
        \item If $c^* \neq c$, find $\adv$'s $\Htag(\pw,pk,c^*,K^*)$ query whose result is $\tau^*$. If there is none or more than one, set $SK := \bot$. (This matches Game 2.)
        \item \fbox{Query $\PC(sk,c^*,K^*)$}. \\
...
\emph{{\color{red}This game corresponds to game $\mathsf{G}_8$ in \cite{EPRINT:ArrBarJar25}. Note that if we reduce to the OW-PCA property of KEM where the adversary can query the $\PC$ oracle polynomially many times, then step 4 of the reduction above can query $\PC(sk,c,(K')^*)$ $\qkey+\qtag$ times to check which of $\adv$'s query (if any) triggers $\Query_1$, hence avoiding the $\qkey+\qtag$ loss in its advantage. This is the approach taken by \cite{EPRINT:ArrBarJar25}.}}
...
\begin{enumerate}
  ...
  \item On $(c^*,\tau^*)$ from $\adv$,
      \begin{enumerate}
        \item If $c^* = c$, do what's described in Game 1.
        \item If $c^* \neq c$, find $\adv$'s $\Htag(\pw,pk,c^*,K^*)$ query whose result is $\tau^*$. If there is none or more than one, set $SK := \bot$. (This matches Game 2.)
        \item \fbox{Query $\PC(sk,c^*,K^*)$}. \quad // {\color{blue}This is a valid query since we do this step only if $c^* \neq c$} \\
            If the result is 1, output $SK := \Htag(\pw,pk,c^*,K^*)$ to $\env$. \\
            If the result is 0, output $SK := \bot$ to $\env$.
      \end{enumerate}
...
\begin{enumerate}
  \item \cite{EPRINT:ArrBarJar25} reduces to ANO-PCA (where the KEM adversary can query its PCA oracle polynomially many times), rather than ANO-PCA1. Unlike in Game 3, here reducing to this stronger assumption does not provide any advantage.
