...
\medskip\noindent\emph{Game 5:} When either $\adv$ or an honest party makes a fresh query $H_2(\pw^*,T)$ (let the result be $t^*$)\footnote{When $P$ makes such a query, it is in the form of $H_2(\pw,T)$ and the result is $t$; when $P'$ makes such a query, it is in the form of $H_2(\pw',T)$ and the result is $t'$. Of course $P'$ makes this query only if $T^* = T$.}, generate $(pk^*,sk^*) \gets \Gen$ and compute
\[
r^* := s \oplus t^*, \quad R^* := T/pk^*.
\]
...
We first upper-bound the aborting probability. For every fresh $H_2(\pw^*,T)$ query, an independent $t^* \gets \{0,1\}^{3n}$ value is sampled, which results in an independent $r^* \gets \{0,1\}^{3n}$ (regardless of the distribution of $s$). As such, there are up to $q_2+2$ independent and uniformly random $r^*$ values. The probability that one of $\adv$'s $q_1$ $H_1(\pw^*,r^*)$ query inputs happens to be any of them is upper-bounded by $q_1(q_2+2)/2^{3n}$.

Now suppose the aborting condition does not happen. The difference between Game 4 and Game 5 is that Game 4 samples $H_1(\pw^*,r^*)$ uniformly at random, whereas Game 5 defines it as $T/pk^*$. Note that $pk^*$ is not used anywhere else in the game, so the distinguishing advantage between Game 4 and Game 5 can be reduced to the PR-PK property of KEM. We only sketch the reduction $\red_\text{PR-PK}$ here: $\red_\text{PR-PK}$, on input $pk^*$ (which is either an honestly generated public key or a uniformly random value), samples $i \gets [q_2+2]$, and answers the (either $\adv$'s or an honest parties') first $i-1$ $H_2$ queries as in Game 4 and the last $q_2-i+2$ $H_2$ queries as in Game 5; for the $i$-th $H_2$ query, $\red_\text{PR-PK}$ computes
\[
r^* := s \oplus t^*, \quad R^* := T/pk^*
\]
...
A standard hybrid argument shows that (assuming the aborting condition does not happen) $\env$'s distinguishing advantage between Game 4 and Game 5 is upper-bounded by $q_2\cdot\Adv_{\red_\text{PR-PK}}^\text{PR-PK}$.\footnote{Note that here we cannot make $q_2$ separate reductions to the PR-PK property, and must reduce to PR-PK ``in one blow''. Furthermore, the calculation of the reduction's winning probability is more complicated than the normal case where the reduction wins as long as its guess is correct. This is similar to, say, why composing a PRG polynomial times yields another PRG; see, e.g., \cite[Sec~3.4.3]{book:BonSho23} for details.} Overall, we have
\[
\Dist_\env^{4,5} \leq (q_2+2)\Adv_{\red_\text{PR-PK}}^\text{PR-PK} + \frac{q_1(q_2+2)}{2^{3n}}.
\]
...
\medskip\noindent\emph{Game 6:} Again in the case where $\pw \neq \pw' \land (s^*,T^*) = (s,T)$, sample $SK',\tau' \gets \{0,1\}^n$.
...
On input $(pk',c)$,
\begin{enumerate}
  \item Invoke $\env$. On $(\NewSession, sid, P, P', \pw, \text{``initiator''})$ from $\env$, run $P$'s algorithm to compute $(s,T)$. {\color{blue}(This involves making an $H_2(\pw,T)$ query on behalf of $P$.)} Send $(s,T)$ to $\adv$.
...
\paragraph{Remark.}
It is very subtle why the reduction needs to guess the index $j$ and lose a factor of $q_2+1$ (and we applaud \cite{EPRINT:ArrBarJar25} for doing it correctly). The reason is as follows. Consider the following environment $\env$:
\begin{enumerate}
...
\end{enumerate}
To simulate Game 5/6 correctly, $\red_\text{OW1}$ must use its input $pk'$ to program $H_1(\pw',r') := T/pk'$ in step 3. The problem is that \emph{$\red_\mathrm{OW1}$ does not know $\pw'$ until step 4, so it has no idea which $H_1(\pw',r')$ query in step 3 needs to be programmed using $pk'$}! The only getaround is to make a guess over all $H_2$ queries, which incurs a $q_2+1$ loss.\footnote{It is $q_2+1$ rather than $q_2+2$, since $P$'s $H_2(\pw,T)$ query is guaranteed to be different from $H_2(\pw',T)$ (otherwise $\red_\text{OW1}$ would abort in step 3) and thus can be excluded from the guess.} (If we require $\env$ to always initiate $P$ and $P'$'s sessions simultaneously, this loss can be avoided since in step 3 $\red_\mathrm{OW1}$ must have already learned $\pw'$.)
