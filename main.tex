...
\begin{itemize}
  \item If $K^* = K$, set $SK := \Hkey(\pw,pk,c^*,K)$ (i.e., there is no change from Game 1).
  \item If $K^* \neq K$, set $SK := \bot$.
\end{itemize}
...
Let $\Query_3$ be the event that $\adv$ queries \emph{either} $\Hkey(\pw',pk',c,K')$ \emph{or} $\Htag(\pw',\allowbreak pk',c,K')$. Clearly Game 14 and Game 15 are identical unless $\Query_3$ happens.
