...
r^* := s \oplus t^*, \quad R^* := T/pk^*.
\]
($(s,T)$ are the honest $P$-to-$P'$ message.) If $H_1(\pw^*,r^*)$ has already been defined and it is not $R^*$, abort. Otherwise program $H_1(\pw^*,r^*) := R^*$, and store $\left<\pw^*,sk^*\right>$.
...
\begin{itemize}
  \item If $H_2(\pw,T)$ has now been defined (via a query by $\adv$), there must be a corresponding record $\left<\pw,sk\right>$ (see Game 5).
...
\t \t SK := \bot \< \< \< \< \\
\text{otherwise } \< \< \< \< \\
\t \text{fetch }\adv\text{'s }\Htag(\pw,pk,c^*,K^*) = \tau^*\text{ query} \< \< \< \< \\
\t \text{if none or more than one} \< \< \< \< \\
\t \t SK := \bot \\
\t \text{fetch }\left<\pw,sk\right>\text{ record} \< \< \< \< \\
\t \text{if none} \< \< \< \< \\
...
We conclude that
\[
\Dist_\env^{10,11} \leq \frac{\qtag(\qtag-1)}{2^{2n+1}}.
\]
